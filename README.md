...
모양과 책임이 같은 페이지는 같은 경로를 갖게 하는게 좋다. => 같은 "/" 홈 디렉토리에 쿼리스트링을 이용해서 구분한다.

경로를 이용해서 데이터를 나타내려고 하면 어려움이 생긴다.

---

### I22 요기요

경로(/)는 계층구조이기때문에(부모-자식), 데이터를 경로로 나타내려고 하는 건 좋지 않다.

경로로 표시하기 쉬운 데이터가 있고 쿼리스트링으로 표현하기 쉬운 데이터가 있다.

계층 구조를 나타내는 데이터는 경로에 포함시키기 좋고,

계층 구조가 아니라면 쿼리스트링으에 포함시키기 좋다.

---

### I23

쿼리스트링 문자열이 통째로 location.search에 들어있다.

URLSearchParams : 최신 브라우저에서만 적용.

=> [URLSearchParams 기능을 갖고있는 라이브러리 : qs](https://www.npmjs.com/package/qs)

---

### I24

카테고리에 들어간 상태에서 다른 카테고리를 누르면 작동하지 않는다.

prop이나 state가 달라지면 render메소드가 다시 불러와지긴 하지만 componentDidMount가 다시 실행되지않는다. => 다른 카테고리를 눌러도 서버로부터 새로 데이터를 받아오지 않는다.

key를 붙여서 완전히 새로운 컴포넌트(ProductList)를 다시 그릴수 있도록 한다 => 다시 그리면 서버에서 데이터를 다시 받아온다.(componentDidMount가 실행되므로)
